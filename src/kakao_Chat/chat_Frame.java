			...
			if (messages.isPresent()) {
				...
				for (Message message : messages.get()) {
					...
					if (message.getSendFrom().equals(Login_Frame.userName)) {
                        ...
                        else if(message.getBody().contains("*23#")){
                            System.out.println("제발");
                            DataInputStream dis = new DataInputStream(s.getInputStream());
                            dis.readFully(message.getBody().getBytes());
                            ByteArrayInputStream inputStream = new ByteArrayInputStream(message.getBody().getBytes());
...
//                        }
                            printImageInHistory(img, message.getTime());
                        }
						else
...
//                             img = (ImageIcon)ois.readObject();
//                        }
                            printImageInHistory_Left(img, message.getSendFrom(), message.getTime());
                        } else
    ...
    public void printImageInHistory_Left(ImageIcon img, String time, String Username) throws IOException {
        ImageIcon Image = EditImage(img,200);
        //프로필 사진 패널
        JPanel profile_panel = new JPanel();
        profile_panel.setBackground(new Color(186,206,224));
    ...
    public void printImageInHistory(ImageIcon img, String time){
        ImageIcon edit = img;
        int height = edit.getIconHeight();
        JPanel chat_view = new JPanel();
